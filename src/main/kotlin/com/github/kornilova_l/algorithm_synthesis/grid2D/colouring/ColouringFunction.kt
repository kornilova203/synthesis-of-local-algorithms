...
class ColouringFunction constructor(private val tileColours: Map<Tile, Int>, val k: Int) {
    ...
    fun getGraphColoured(grid2d: Grid2D): Array<IntArray>? {
        ...
        for (i in independentSet.indices) {
            for (j in independentSet[i].indices) {
                val tile = Tile(independentSet, i, j, tileColours.entries.first().key.n, tileColours.entries.first().key.m)
